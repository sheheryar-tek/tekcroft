/**
 * Faithful extract of tekcroft-seo-audit (7).html → Next.js assets.
 * No redesign: all original styles + body + scripts (images externalized).
 * Run: node scripts/extract-seo-audit-services.mjs
 */
import fs from "fs";
...
const SRC =
  process.env.TEKCROFT_SEO_AUDIT_HTML ||
  "c:\\Users\\Super\\Desktop\\TK FINAL\\Services\\SEO AUDIT\\tekcroft-seo-audit (7).html";
...
let css =
  `/* Exact extract from tekcroft-seo-audit (7).html — do not redesign */\n` +
  styleBlocks.join("\n\n");
...
const outCss = path.join(ROOT, "app", "seo-audit-services.css");
...
const outHtml = path.join(ROOT, "lib", "seo-audit-services-body.html");
...
const bundled = scripts.map((m, i) => {
  ...
  let code = m[2];
  // SEO Audit item → this route
  if (id === "mm-script") {
    code = code.replace(
      /\{name:"SEO Audit Services", href:"\/#services"/g,
      '{name:"SEO Audit Services", href:"/services/seo-audit-services"'
    );
    code = code.replace(
      /\{name:"SEO Audit", href:"\/#services"/g,
      '{name:"SEO Audit Services", href:"/services/seo-audit-services"'
    );
    code = code.replace(
...
const outJs = path.join(ROOT, "public", "tekcroft-seo-audit-services.js");
...
// LCP preload: prefer hero shot from body, else shared hero-1
const heroSrc =
  (body.match(/class="hs-bg"[\s\S]*?<img[^>]+src="([^"]+)"/i) ||
    body.match(/fetchpriority="high"[^>]*src="([^"]+)"/i) ||
    body.match(/src="([^"]+)"[^>]*fetchpriority="high"/i) ||
    [])[1] || "/images/hero-1.webp";
fs.writeFileSync(
