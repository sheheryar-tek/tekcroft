/**
 * Extract AI Agent Development standalone HTML → Next.js App Router assets.
 * Run: node scripts/extract-ai-agent-development.mjs
 */
import fs from "fs";
...
function pruneUnusedImages(css, body) {
  const blob = css + "\n" + body;
  const refs = new Set(
    [...blob.matchAll(/\/images\/(aag-[a-z0-9._-]+\.(?:webp|svg))/gi)].map((m) => m[1])
  );
  let removed = 0;
  for (const file of fs.readdirSync(IMG)) {
    if (!file.startsWith("aag-")) continue;
...
css =
  `/* AI Agent Development — design from source HTML; chrome in tekcroft.css */\n` +
  css +
  GROUNDS +
  TYPE_SCALE;
fs.writeFileSync(path.join(ROOT, "app", "ai-agent-development.css"), css);
...
if (navEnd < 0) throw new Error("AAG nav end not found");
...
if (!heroSrc) {
  const bg = css.match(/\.hs-bg[^}]*url\("(\/images\/aag-[^"]+)"/i);
  if (bg) heroSrc = bg[1];
}
...
fs.writeFileSync(path.join(ROOT, "lib", "ai-agent-development-body.html"), body);
fs.writeFileSync(
  path.join(ROOT, "lib", "ai-agent-development-lcp.json"),
...
fs.writeFileSync(
  path.join(ROOT, "public", "tekcroft-ai-agent-development.js"),
  js
);
...
fs.writeFileSync(
  path.join(ROOT, "lib", "ai-agent-development-meta.json"),
  JSON.stringify(
    {
      title:
        (titleMatch && titleMatch[1].trim()) ||
        "AI Agent Development Services | Tekcroft",
      description:
        (descMatch && descMatch[1].trim()) ||
        "Custom AI agent development for real business workloads.",
      canonical:
        "https://www.tekcroft.com/services/ai-agent-development-services",
    },
    null,
    2
  )
