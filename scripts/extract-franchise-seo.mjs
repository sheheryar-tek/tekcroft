/**
 * Extract Franchise SEO Services standalone HTML → Next.js App Router assets.
 * Run: node scripts/extract-franchise-seo.mjs
 */
import fs from "fs";
...
const SRC =
  process.env.TEKCROFT_FSEO_HTML ||
  "c:\\Users\\Super\\Desktop\\TK FINAL\\Services\\Franchise SEO Services\\tekcroft-franchise-seo (4).html";
const PREFIX = "fseo";
...
.ct-hero, .hs, #hero-1, #top, #faq, #why, #services-2, #what-is, #signs, #governance, #duplicate, #duplicate-v2, #compare, #work, #proof {
...
const GROUNDS = `
/* Section grounds — white / tint; franchise uses --bg (true white) for even bands */
#what-is,
#services-2,
#governance,
#compare,
...
function pruneUnusedImages(css, body) {
  const blob = css + "\n" + body;
  const refs = new Set(
    [...blob.matchAll(/\/images\/(fseo-[a-z0-9._-]+\.(?:webp|svg))/gi)].map((m) => m[1])
  );
  let removed = 0;
  for (const file of fs.readdirSync(IMG)) {
    if (!file.startsWith("fseo-")) continue;
...
css =
  `/* Franchise SEO Services — design from source HTML; chrome in tekcroft.css */\n` +
  css +
  GROUNDS +
  TYPE_SCALE;
fs.writeFileSync(path.join(ROOT, "app", "franchise-seo.css"), css);
...
if (navEnd < 0) throw new Error("FSEO nav end not found");
...
if (!heroSrc) {
  const bg = css.match(/\.hs-bg[^}]*url\("(\/images\/fseo-[^"]+)"/i);
  if (bg) heroSrc = bg[1];
}
...
fs.writeFileSync(path.join(ROOT, "lib", "franchise-seo-body.html"), body);
fs.writeFileSync(
  path.join(ROOT, "lib", "franchise-seo-lcp.json"),
...
fs.writeFileSync(
  path.join(ROOT, "public", "tekcroft-franchise-seo.js"),
  js
);
...
fs.writeFileSync(
  path.join(ROOT, "lib", "franchise-seo-meta.json"),
  JSON.stringify(
    {
      title:
        (titleMatch && titleMatch[1].trim()) ||
        "Franchise SEO Services | Tekcroft",
      description:
        (descMatch && descMatch[1].trim()) ||
        "Franchise SEO programs that put every location on the map while keeping your national brand authority intact.",
      canonical:
        "https://www.tekcroft.com/services/franchise-seo-services",
    },
    null,
    2
  )
