/**
 * Extract AI Chatbot Development standalone HTML → Next.js App Router assets.
 * Run: node scripts/extract-ai-chatbot-development.mjs
 */
import fs from "fs";
...
const SRC =
  process.env.TEKCROFT_ACB_HTML ||
  "c:\\Users\\Super\\Desktop\\TK FINAL\\Services\\AI Chatbot Development\\tekcroft-ai-chatbot (2).html";
const PREFIX = "acb";
...
.ct-hero, .hs, #hero-1, #top, #faq, #why, #services-2, #systems, #types, #stack, #work, #reviews {
...
const GROUNDS = `
/* Section grounds — white / tint alternation; #whyus stays tinted so
   the white folder tabs read against the section ground */
#services-2,
#types,
#industries,
#reviews,
...
function pruneUnusedImages(css, body) {
  const blob = css + "\n" + body;
  const refs = new Set(
    [...blob.matchAll(/\/images\/(acb-[a-z0-9._-]+\.(?:webp|svg))/gi)].map((m) => m[1])
  );
  let removed = 0;
  for (const file of fs.readdirSync(IMG)) {
    if (!file.startsWith("acb-")) continue;
...
css =
  `/* AI Chatbot Development — design from source HTML; chrome in tekcroft.css */\n` +
  css +
  GROUNDS +
  TYPE_SCALE;
fs.writeFileSync(path.join(ROOT, "app", "ai-chatbot-development.css"), css);
...
if (navEnd < 0) throw new Error("ACB nav end not found");
...
if (!heroSrc) {
  const bg = css.match(/\.hs-bg[^}]*url\("(\/images\/acb-[^"]+)"/i);
  if (bg) heroSrc = bg[1];
}
...
fs.writeFileSync(path.join(ROOT, "lib", "ai-chatbot-development-body.html"), body);
fs.writeFileSync(
  path.join(ROOT, "lib", "ai-chatbot-development-lcp.json"),
...
fs.writeFileSync(
  path.join(ROOT, "public", "tekcroft-ai-chatbot-development.js"),
  js
);
...
fs.writeFileSync(
  path.join(ROOT, "lib", "ai-chatbot-development-meta.json"),
  JSON.stringify(
    {
      title:
        (titleMatch && titleMatch[1].trim()) ||
        "AI Chatbot Development Services | Tekcroft",
      description:
        (descMatch && descMatch[1].trim()) ||
        "AI chatbot development services that connect to your real systems.",
      canonical:
        "https://www.tekcroft.com/services/ai-chatbot-development-services",
    },
    null,
    2
  )
