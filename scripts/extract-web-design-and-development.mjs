/**
 * Extract Web Design and Development standalone HTML → Next.js App Router assets.
 * Run: node scripts/extract-web-design-and-development.mjs
 */
import fs from "fs";
...
const SRC =
  process.env.TEKCROFT_WDD_HTML ||
  "c:\\Users\\Super\\Desktop\\TK FINAL\\Services\\Web Design and Development\\tekcroft-web-design (2).html";
const PREFIX = "wdd";
...
const SHARED = {
  ...
  "cn-shot": "/images/cn-web-design.webp",
...
/* Keep the plate photograph visible — translucent wash (not opaque #0b1220) */
#services-2{ --cn-shot:url("/images/cn-web-design.webp"); }
...
function pruneUnusedImages(css, body) {
  const blob = css + "\n" + body;
  const refs = new Set(
    [...blob.matchAll(/\/images\/(wdd-[a-z0-9._-]+\.(?:webp|svg))/gi)].map((m) => m[1])
  );
  let removed = 0;
  for (const file of fs.readdirSync(IMG)) {
    if (!file.startsWith("wdd-")) continue;
...
css =
  `/* Web Design and Development — design from source HTML; chrome in tekcroft.css */\n` +
  css +
  GROUNDS +
  TYPE_SCALE;
fs.writeFileSync(path.join(ROOT, "app", "web-design-and-development.css"), css);
...
if (navEnd < 0) throw new Error("WDD nav end not found");
...
if (!heroSrc) {
  const bg = css.match(/\.hs-bg[^}]*url\("(\/images\/wdd-[^"]+)"/i);
  if (bg) heroSrc = bg[1];
}
...
fs.writeFileSync(path.join(ROOT, "lib", "web-design-and-development-body.html"), body);
fs.writeFileSync(
  path.join(ROOT, "lib", "web-design-and-development-lcp.json"),
...
fs.writeFileSync(
  path.join(ROOT, "public", "tekcroft-web-design-and-development.js"),
  js
);
...
fs.writeFileSync(
  path.join(ROOT, "lib", "web-design-and-development-meta.json"),
  JSON.stringify(
    {
      title:
        (titleMatch && titleMatch[1].trim()) ||
        "Web Design and Development Services | Tekcroft",
      description:
        (descMatch && descMatch[1].trim()) ||
        "Custom website design and development that makes it easy for visitors to find what they need and trust your business.",
      canonical:
        "https://www.tekcroft.com/services/web-design-and-development-services",
    },
    null,
    2
  )
