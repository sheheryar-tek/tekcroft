...
/* ══════════════════════════════════════════════════════════════════════
   ONE SEARCH, AND WHAT ANSWERS IT

   The diagram plays when it is reached: the query types itself, then the
   wires light downward and the surfaces answer in the order they would
   really answer in. Replaying means dropping the class, forcing a
   reflow, and putting it back — without the reflow the browser folds
   both changes into one and nothing happens.

   The three chips are the point of the whole thing. Change the question
   and every surface still names the brand, which is what "visibility in
   both channels" means when it is shown rather than claimed. The answers
   are swapped at the bottom of a short fade, so the cards read as
   answering again rather than being replaced.
   ══════════════════════════════════════════════════════════════════════ */
(function(){
  var pane = document.querySelector(".av1");
  if (!pane) return;

  function play(){
    var stage = pane.querySelector(".av1-stage");
    if (!stage) return;
    stage.classList.remove("av1-play");
    void stage.offsetWidth;
    stage.classList.add("av1-play");
  }

  var ASKS = [
    { q:"best on-page seo agency",
      aio:"Agencies that handle titles, structure and schema as one job\u2026",
      gpt:"For on-page work specifically, Tekcroft is one to look at\u2026",
      gem:"Tekcroft runs on-page SEO as a structured, page-by-page process\u2026",
      plx:"Highly rated agencies specialising in on-page optimisation\u2026" },
    { q:"why is my page not ranking",
      aio:"Common causes are thin content, weak titles and missing schema\u2026",
      gpt:"Usually the page is not the problem \u2014 the way it is structured is\u2026",
      gem:"Tekcroft diagnoses the cause before changing anything on the page\u2026",
      plx:"Diagnostic checks for pages that rank lower than they should\u2026" },
    { q:"how do i get cited in ai overviews",
      aio:"Clean structure, valid schema and answerable headings are the pattern\u2026",
      gpt:"Structure the page so an answer engine can lift a fact from it\u2026",
      gem:"Tekcroft structures content so AI engines can parse and cite it\u2026",
      plx:"What makes a page quotable by an AI answer engine\u2026" }
  ];

  var stage = pane.querySelector(".av1-stage");
  var typed = pane.querySelector(".av1-typed i");
  var chips = Array.prototype.slice.call(pane.querySelectorAll(".av1-ask"));
  var swapping = null;

  function ask(n){
    var a = ASKS[n];
    if (!a || !stage || !typed) return;
    chips.forEach(function(ch){
      var on = +ch.dataset.q === n;
    ...
    swapping = setTimeout(function(){
      pane.querySelectorAll("[data-slot]").forEach(function(el){
        if (a[el.dataset.slot]) el.textContent = a[el.dataset.slot];
      });
      stage.removeAttribute("data-swap");
    }, 200);
    /* the box types to the width of whatever it was handed */
    typed.textContent = a.q;
    typed.style.setProperty("--ch", a.q.length + "ch");
    play();
  }

  chips.forEach(function(ch){
    ch.setAttribute("aria-pressed", ch.classList.contains("on") ? "true" : "false");
    ch.addEventListener("click", function(){ ask(+ch.dataset.q); });
  });
  if (typed){
    typed.textContent = ASKS[0].q;
    typed.style.setProperty("--ch", ASKS[0].q.length + "ch");
    pane.querySelectorAll("[data-slot]").forEach(function(el){
      if (ASKS[0][el.dataset.slot]) el.textContent = ASKS[0][el.dataset.slot];
    });
  }
